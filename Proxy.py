با توجه به اینکه شما درخواست کرده‌اید تغییرات لازم را در کدی که قبلاً اصلاح کرده بودیم، اعمال کنم و با توجه به خلاصه‌ای که ارائه دادید، متوجه شدم که **آخرین نسخه کامل کد `Proxy.py` شما نیاز به اعمال دو تغییر متنی زیر در خروجی نمایش لیست V2Ray دارد**:

1.  اضافه کردن متن: `برای بازگشت از دکمه «بازگشت» پایین استفاده کنید.` (که قبلاً در تابع `format_proxy_grid_text` بود و باید به انتهای خروجی V2Ray هم اضافه شود).
2.  اضافه کردن متن نکته: `نکته: کل متن داخل بلوک بالا (شامل شماره‌ها و کانفیگ‌ها) با یک کلیک کپی می‌شود. لطفاً بعد از کپی، شماره‌ها را در صورت نیاز حذف کنید.`

من این تغییرات را در تابع `format_v2ray_list` اعمال می‌کنم و همچنین متغیر `note` را که قبلاً در کد وجود داشت اما تعریف نشده بود (و احتمالاً به همین دلیل باعث خطای سینتکسی در اجرای قبلی شده بود)، حذف یا جایگزین می‌کنم. همچنین، چون شما تأیید کردید که **تمام کدهای قبلی (رفع خطای سینتکسی و اجرای Flask/Bot)** را در ذهن دارم، تمرکز را روی اعمال این دو تغییر متنی جدید می‌گذارم.

در زیر، **نسخه کامل و نهایی کد `Proxy.py`** با اعمال این اصلاحات ارائه شده است. شما می‌توانید این کد را به طور کامل جایگزین فایل قبلی خود کنید.

### تغییرات اعمال شده در کد:

1.  **تابع `format_v2ray_list` اصلاح شد:**
    *   متن بازگشت از تابع `back_and_group_kb()` به انتهای خروجی اضافه شد.
    *   متن نکته درخواستی شما به انتهای خروجی اضافه شد.
    *   متغیر تعریف نشده `note` حذف شد.
2.  **تابع `format_proxy_grid_text` اصلاح شد:**
    *   متن بازگشت به صورت استاندارد حفظ شد.
3.  **توابع مربوط به Flask/Threading برای اجرای همزمان** که قبلاً برای رفع مشکل Render اعمال شده بود، دست نخورده باقی می‌ماند.

---
### کد کامل اصلاح شده `Proxy.py`
